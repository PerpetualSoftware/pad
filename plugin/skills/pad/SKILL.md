...
- **A `PAD (resync) ...` line means the monitor may have missed notifications it cannot replay** (the server cannot vouch for that span) — a Redis failover, an id-space restart, a slow stretch the server could not buffer. It names no item, because the server cannot say which ones it lost. Don't treat your picture of watched items as current: when the work in front of you depends on one, re-read it (`pad watch list` shows what you are watching, then `pad --workspace <ws> item show <ref>`). It is not a write trigger — the never-write rule above still applies — and repeated resyncs with no notification between them arrive as one line.
