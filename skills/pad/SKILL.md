...
**Never run `pad workspace init` blind.** On a machine that is configured but not authenticated it drops into an interactive browser auth flow: it prints a URL and polls every 2s, waiting for a human to approve in a browser. In the ordinary case that ends on its own — the server expires the auth session after ~5 minutes (~20 for first-admin setup) — but it still blocks your tool call for minutes with no non-interactive way through. (The poll has no wall-clock limit of its OWN; it stops only when the server says `expired`, so if the server becomes unreachable after the session is created, it keeps polling. Narrow, but it is why "it eventually times out" is not a reason to risk it.)
