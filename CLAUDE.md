...
- `GET/PATCH/DELETE /workspaces/{ws}/items/{slug}` — item by slug
  ...
  - **Write responses may also carry `warnings.content_outcome`** (BUG-2995) — on a 200 it takes exactly one value, `applied_pending_flush`: the content reached the live collaborative document (a browser tab has the item open) and `items.content` has not caught up. The response's `content` is then the markdown as **sent**, not as stored, and a read inside that window answers with the PREVIOUS content — the lag, not a failed write. The stored form is not byte-identical to the sent form either: the markdown round-trips through the editor, so bullet markers, emphasis characters, list numbering and blank-line runs normalise (measured — `web/src/lib/collab/bug2995Roundtrip.svelte.test.ts`). No duration is claimed anywhere, because it is not established that the flush always lands (BUG-3000). The CLI prints this to **stderr**, like the other write warnings
