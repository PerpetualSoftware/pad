...
var padItemSchemaParams = []ParamDef{
	...
	{Name: "field", Type: "array<string>", Description: "Custom field setters for SCHEMA-DECLARED fields without a dedicated top-level param. Array of \"key=value\" strings (e.g. [\"due_date=2026-06-01\",\"effort=l\"]). For status/priority/category/parent/role/assign/tags use the dedicated top-level param instead. Optional for: create, update, list filter, move. System-metadata keys (implementation_notes, decision_log, github_pr, convention) are REFUSED on update (code=validation_failed, BUG-2627) and on move/copy (code=malformed_override, BUG-2674). On CREATE they are not blocked — that door is shared with Pad's own writers — but do not set them there either: they are written by action=note / action=decide and the GitHub link flow, and a raw field write bypasses those guards and stores a value Pad cannot read back, which hides the existing entries and makes note/decide refuse on that item until it is repaired."},
