...
var padItemSchemaParams = []ParamDef{
	...
	{Name: "field", Type: "array<string>", Description: "Custom field setters for SCHEMA-DECLARED fields without a dedicated top-level param. Array of \"key=value\" strings (e.g. [\"due_date=2026-06-01\",\"effort=l\"]). For status/priority/category/parent/role/assign/tags use the dedicated top-level param instead. Optional for: create, update, list filter, move. System-metadata keys are REFUSED here on update (BUG-2627) and on move/copy (BUG-2674) — both surface as code=validation_failed, with the server's own code (validation_error / malformed_override) in the message. Each has its own writer: implementation_notes -> action=note, decision_log -> action=decide, github_pr -> the GitHub link flow, convention -> library activation (no update path at all). On CREATE they are NOT blocked, because that door is shared with those writers — but do not set them there either: a raw write to implementation_notes / decision_log stores a value Pad cannot read back, which hides the existing entries AND makes note/decide refuse on that item until it is repaired; a raw write to github_pr / convention silently overwrites what the real writer maintains."},
