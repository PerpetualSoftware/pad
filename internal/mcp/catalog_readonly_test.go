...
func TestReadOnlyCatalog_ActionsMatchCmdhelp(t *testing.T) {
	...
	expected := map[[2]string][]string{
		...
		{"pad_item", "claim"}:         {"item", "claim"},
		{"pad_item", "release"}:       {"item", "release"},
...
func liveCmdhelpDoc(t *testing.T) *cmdhelp.Document {
	...
	return &cmdhelp.Document{
		...
		Commands: map[string]cmdhelp.Command{
			...
			"item claim": {
				Summary: "claim an execution lease",
				Args:    mkArgs("ref"),
				Flags:   mkFlags("workspace", "holder", "ttl"),
			},
			"item release": {
				Summary: "release an execution lease",
				Args:    mkArgs("ref"),
				Flags:   mkFlags("workspace", "holder"),
			},
			"item star": {
				Summary: "star item",
				Args:    mkArgs("ref"),
