...
1. Confirm the playbook is activated. Call ` + "`pad_playbook`" + ` with ` + "`action: list`" + ` and look for ` + "`invocation_slug=onboard`" + ` with ` + "`status=active`" + `. If an onboard entry exists but is NOT active (draft/deprecated), reactivate it in place — ` + "`pad_item`" + ` with ` + "`action: update`" + `, its ref, and ` + "`field: [\"status=active\"]`" + ` — because ` + "`invocation_slug`" + ` is workspace-unique and activating from the library beside an existing entry creates a duplicate or fails on the slug. Only when NO onboard entry exists at all, activate it from the library — call ` + "`pad_library`" + ` with ` + "`action: activate`" + ` and ` + "`title: \"Onboard a workspace\"`" + ` (no shell needed) — then re-list. (The user can also activate it from the library in the web UI: ` + "`/{ws}/library?tab=playbooks`" + `.)
2. Load the body. Call ` + "`pad_playbook`" + ` with ` + "`action: get`" + ` and ` + "`ref: onboard`" + ` (this returns the full body; it is side-effect-free).
3. Follow the body's instructions. It teaches you the surface-agnostic interview: discover the domain, propose collections, adapt seeded conventions/playbooks to the project's actual tooling, suggest roles, seed a first item. The body is the source of truth — this prompt is just the dispatcher. One routing fact worth knowing before you start: the body's ` + "`auto`" + ` mode sends any workspace with user-created items to ` + "`revisit`" + `; if the user says the workspace was never really set up, bind an explicit ` + "`mode=build`" + ` or ` + "`mode=audit`" + ` — the playbook honors the override.
