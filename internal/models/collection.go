...
type FieldDef struct {
	...
	// AbandonedOptions (BUG-2347) is the subset of TerminalOptions that
	// closes an item WITHOUT delivering it (cancelled, rejected, overturned,
	// withdrawn…). Completed-work views (changelog, standup, report
	// throughput) count terminal − abandoned. When a field declares none,
	// the global NegativeTerminals names are the fallback, so a collection
	// that never sets it behaves as before. Validated as a subset of
	// TerminalOptions on collection create/update.
