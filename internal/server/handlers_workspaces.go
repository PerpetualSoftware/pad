...
func (s *Server) maybeAutoAddCreatorConnection(r *http.Request, workspaceID string) {
	...
	if err := s.store.AddCreatedWorkspaceIfPermitted(requestID, workspaceID); err != nil {
		...
		// store, so an error here is an unexpected store error: rare,
		// possibly transient (a busy SQLite), and worth logging so ops
		// sees it. The workspace itself is already created either way.
